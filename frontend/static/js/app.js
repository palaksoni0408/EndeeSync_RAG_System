...
const ApiClient = {
    ...
    // Clear all data
    async clearAll() {
        return this.request('/api/v1/clear-all', {
            method: 'DELETE',
        });
    },
};
    ...
    init() {
        ...
        btn.addEventListener('click', async () => {
            ...
            const confirmed = confirm(
                '⚠️ WARNING: This will permanently delete ALL stored embeddings and notes.\n\n' +
                'This action cannot be undone. Are you sure you want to continue?'
            );

            if (!confirmed) return;

            // Add hover effect style if not exists
            btn.style.opacity = '0.7';
            btn.disabled = true;

            try {
                await ApiClient.clearAll();
                UI.showToast('All data cleared successfully! Database has been reset.', 'success');

                // Refresh notes list
                NotesSection.loadNotes();
            } catch (error) {
                UI.showToast(`Clear failed: ${error.message}`, 'error');
            } finally {
                btn.style.opacity = '1';
                btn.disabled = false;
            }
